...
$baseDir = "D:\.gemini\antigravity\scratch\AccessibleVideoEditor\app\src\main\res"

foreach ($lang in $translations.Keys) {
    $filePath = Join-Path $baseDir "values-$lang\strings.xml"
    if (-Not (Test-Path $filePath)) { continue }

    [xml]$xml = Get-Content $filePath -Encoding UTF8
    
    $langTranslations = $translations[$lang]
    foreach ($key in $langTranslations.Keys) {
        $value = $langTranslations[$key]
        $node = $xml.SelectSingleNode("//string[@name='$key']")
        if ($node) {
            ...
            $xml.resources.AppendChild($newNode) > $null
        }
    }

    # Save cleanly with UTF-8
    $writer = [System.Xml.XmlTextWriter]::new($filePath, [System.Text.Encoding]::UTF8)
