...
class CompressVideoFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val title = AppStrings.get(requireContext(), R.string.string_125)
    ...
    private fun processVideo(uri: Uri) {
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            ...
            val tempFile = MediaUtils.copyUriToTempFile(requireContext(), uri, "temp_compress_${System.currentTimeMillis()}.mp4")
            if (tempFile != null) {
                val outputFile = File(requireContext().cacheDir, "output_compressed_${System.currentTimeMillis()}.mp4")
                
                val success = FFmpegProcessor.compressVideo(
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (success) {
                        ...
                        val finalUri = MediaUtils.saveVideoToGallery(requireContext(), outputFile, "compressed_video.mp4", "video/mp4")
                        if (finalUri != null) {
                            HistoryManager.saveToHistory(requireContext(), com.example.accessiblevideoeditor.media.HistoryItem(uriString = finalUri.toString(), name = "compressed_video.mp4", type = "video", timestamp = System.currentTimeMillis()))
                        }
