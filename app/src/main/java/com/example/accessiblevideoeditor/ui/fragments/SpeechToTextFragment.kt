...
class SpeechToTextFragment : Fragment() {
    ...
    private fun transcribeAudio() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            try {
                ...
                if (apiKey.isBlank()) {
                    transcribedText = if (currentContext != null) AppStrings.get(currentContext, R.string.string_3) else "Gemini API key is missing."
                    ...
                    val mimeType = currentContext.contentResolver.getType(uri) ?: "audio/mpeg"
                    val inputContent = content {
                        blob(mimeType, bytes)
                        text(AppStrings.get(currentContext, R.string.string_2))
                    }
                    transcribedText = withContext(Dispatchers.IO) {
                        model.generateContent(inputContent).text ?: (if (currentContext != null) AppStrings.get(currentContext, R.string.string_71) else "لم يتم العثور على نص.")
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
                ...
                if (currentContext != null) {
                    if (errorMsg.contains("503") || errorMsg.contains("high demand") || errorMsg.contains("Unexpected Response")) {
                        transcribedText = AppStrings.get(currentContext, R.string.string_228)
                    } else {
                        transcribedText = AppStrings.get(currentContext, R.string.string_73, errorMsg)
                    }
                } else {
                    transcribedText = "Error: $errorMsg"
                }
            } finally {
    ...
    private fun burnSubtitlesToVideo(uri: Uri, text: String) {
        ...
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            try {
                val tempInput = com.example.accessiblevideoeditor.media.MediaUtils.copyUriToTempFile(safeContext, uri, "sub_input_${System.currentTimeMillis()}")
                if (tempInput != null && tempInput.exists()) {
                    withContext(Dispatchers.Main) {
                        val currentContext = context ?: return@withContext
                        ProcessingManager.startProcessing(AppStrings.get(currentContext, R.string.btn_burn_subtitles))
                    }
                    val isVideo = com.example.accessiblevideoeditor.media.MediaUtils.isVideoFile(safeContext, uri)
                    ...
                    val srtFile = java.io.File(safeContext.cacheDir, "temp_sub_${System.currentTimeMillis()}.srt")
                    srtFile.writeText("1\n00:00:00,000 --> 00:00:${String.format("%02d", durSec.toInt())},000\n$cleanText\n")

                    val outputPath = safeContext.cacheDir.absolutePath + "/subbed_video_${System.currentTimeMillis()}.mp4"
                    val srtPath = srtFile.absolutePath.replace("\\", "/").replace(":", "\\:")
                    val vfFilter = "subtitles='$srtPath'"
                    ...
                    if (success) {
                        com.example.accessiblevideoeditor.utils.FileUtils.saveToGallery(safeContext, java.io.File(outputPath), "video/mp4")
                        withContext(Dispatchers.Main) {
                            val currentContext = context ?: return@withContext
