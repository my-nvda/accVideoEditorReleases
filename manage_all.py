...
class TranslationsManagerTab(wx.Panel):
    def __init__(self, parent, sidebar_ref, active_repo_provider):
        super().__init__(parent)
        self.SetLayoutDirection(wx.Layout_RightToLeft)
        self.sidebar = sidebar_ref
        ...
        search_sizer = wx.BoxSizer(wx.HORIZONTAL)
        search_sizer.Add(wx.StaticText(self, label="بحث عن نص:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.search_input = wx.TextCtrl(self, name="بحث في النصوص")
        self.search_input.Bind(wx.EVT_TEXT, self.on_search)
        search_sizer.Add(self.search_input, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.check_sync_btn = wx.Button(self, label="مطابقة ومزامنة مع نصوص الهاتف 🔄", name="مطابقة النصوص مع الهاتف")
        self.check_sync_btn.Bind(wx.EVT_BUTTON, self.on_check_sync_click)
        search_sizer.Add(self.check_sync_btn, 0)
        
        main_sizer.Add(search_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        self.list_ctrl = wx.ListCtrl(self, style=wx.LC_REPORT | wx.LC_SINGLE_SEL, name="قائمة النصوص")
        self.list_ctrl.InsertColumn(0, "المفتاح (Key)", width=200)
        self.list_ctrl.InsertColumn(1, "النص الحالي (Text)", width=400)
        main_sizer.Add(self.list_ctrl, 1, wx.EXPAND | wx.ALL, 10)
        
        edit_box = wx.StaticBox(self, label="تعديل أو إضافة نص")
        edit_sizer = wx.StaticBoxSizer(edit_box, wx.VERTICAL)
        
        self.sel_key_lbl = wx.StaticText(self, label="المفتاح المحدد: لا يوجد")
        edit_sizer.Add(self.sel_key_lbl, 0, wx.BOTTOM, 5)
        
        self.val_input = wx.TextCtrl(self, style=wx.TE_MULTILINE, size=(-1, 60), name="قيمة النص المحددة")
        edit_sizer.Add(self.val_input, 1, wx.EXPAND | wx.BOTTOM, 10)
        
        form_btn_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.update_btn = wx.Button(self, label="تحديث النص المحدد", name="تحديث النص")
        self.update_btn.Bind(wx.EVT_BUTTON, self.on_update_item)
        form_btn_sizer.Add(self.update_btn, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.add_new_btn = wx.Button(self, label="إضافة مفتاح جديد +", name="إضافة مفتاح جديد")
        ...
        self.save_btn = wx.Button(self, label="حفظ ملف الترجمات محلياً", name="حفظ ملف الترجمات")
        self.save_btn.Bind(wx.EVT_BUTTON, self.on_save_patch)
        save_sizer.Add(self.save_btn, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.upload_btn = wx.Button(self, label="حفظ ورفع التغييرات إلى السحابة ☁️", name="رفع ملف الترجمات")
        ...
        wx.CallAfter(self.run_auto_sync_check)
    ...
    def on_item_selected(self, event):
        ...
        self.sel_key_lbl.SetLabel(f"المفتاح المحدد: {key}")
    ...
    def on_add_new_key(self, event):
        dlg = wx.TextEntryDialog(self, "أدخل اسم المفتاح الجديد (اسم الكلمة):", "إضافة مفتاح")
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def on_save_patch(self, event):
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                json.dump(self.patch_data, f, ensure_ascii=False, indent=2)
            play_success_sound()
            wx.MessageBox(f"تم حفظ الملف بنجاح محلياً! (الإصدار الجديد: {current_ver + 1})", "نجاح الحفظ", wx.OK | wx.ICON_INFORMATION)
        except Exception as e:
            play_error_sound()
            wx.MessageBox(f"فشل حفظ الملف:\n{str(e)}", "خطأ", wx.OK | wx.ICON_ERROR)
    ...
    def on_upload_patch(self, event):
        ...
        try:
            ...
            wx.EndBusyCursor()
            play_success_sound()
            wx.MessageBox("تم رفع التراجم السحابية OTA إلى مستودع GitHub بنجاح!", "تم الرفع بنجاح", wx.OK | wx.ICON_INFORMATION)
        except Exception as e:
            if wx.IsBusy():
                wx.EndBusyCursor()
            play_error_sound()
    ...
    def run_auto_sync_check(self):
        ...
        patch_path = os.path.join(releases_repo, "strings_patch.json")
        local_xml_path = os.path.join(source_repo, "app", "src", "main", "res", "values-ar", "strings.xml")
        
        if not os.path.exists(patch_path) or not os.path.exists(local_xml_path):
            return
        ...
        try:
            ...
            patch_strings = patch_data.get("strings", {})
            missing_keys = []
            mismatched_keys = []
            ...
            if missing_keys or mismatched_keys:
                with StringSyncDialog(self, missing_keys, mismatched_keys, local_strings, patch_data, patch_path) as dlg:
                    if dlg.ShowModal() == wx.ID_OK:
                        self.load_patch_file()
        except Exception as e:
            pass
...
class MainManagerFrame(wx.Frame):
    def __init__(self):
        ...
        self.tab10 = TranslationsManagerTab(self.notebook, self.sidebar, active_repo_provider=self)
        self.tab11 = ComplianceAuditorTab(self.notebook, self.sidebar, active_repo_provider=self)
        ...
        self.notebook.AddPage(self.tab10, "10. إدارة التراجم")
        self.notebook.AddPage(self.tab11, "11. مدقق التوافقية والأمان")
    ...
    def active_repo_changed(self):
        ...
        self.tab10.load_patch_file()

    def on_tab_changed(self, event):
        sel = event.GetSelection()
        if sel in (0, 1, 4, 5, 6, 7, 8, 9, 10):
