...
class FCMNotificationTab(wx.Panel):
    def __init__(self, parent, sidebar_ref, active_repo_provider):
        super().__init__(parent)
        self.SetLayoutDirection(wx.Layout_RightToLeft)
        self.sidebar = sidebar_ref
        ...
        header = wx.StaticText(self, label="بث الإشعارات الفورية (Firebase FCM)")
        header_font = wx.Font(12, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        header.SetFont(header_font)
        main_sizer.Add(header, 0, wx.ALL | wx.ALIGN_CENTER_HORIZONTAL, 10)
        
        key_sizer = wx.BoxSizer(wx.HORIZONTAL)
        key_sizer.Add(wx.StaticText(self, label="ملف مفتاح Firebase (JSON):"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.key_picker = wx.FilePickerCtrl(
            self,
            path=r"C:\Users\AHMED\Downloads\accessiblevideoeditor-dc60f-firebase-adminsdk-fbsvc-ce2e3b5c6e.json",
            message="اختر ملف مفتاح الخدمة JSON",
            wildcard="JSON files (*.json)|*.json",
            style=wx.FLP_DEFAULT_STYLE | wx.FLP_FILE_MUST_EXIST
        )
        key_sizer.Add(self.key_picker, 1, wx.EXPAND)
        main_sizer.Add(key_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        fields_sizer = wx.FlexGridSizer(rows=3, cols=2, vgap=10, hgap=10)
        fields_sizer.AddGrowableCol(1, 1)
        
        fields_sizer.Add(wx.StaticText(self, label="عنوان الإشعار:"), 0, wx.ALIGN_CENTER_VERTICAL)
        self.title_input = wx.TextCtrl(self, value="تحديث جديد متوفر! 🚀", name="عنوان الإشعار")
        fields_sizer.Add(self.title_input, 1, wx.EXPAND)
        
        fields_sizer.Add(wx.StaticText(self, label="نص الرسالة:"), 0, wx.ALIGN_CENTER_VERTICAL)
        self.body_input = wx.TextCtrl(self, value="تم إضافة ميزات وأدوات ميسرة جديدة في تطبيق محرر الفيديو الميسّر.", style=wx.TE_MULTILINE, size=(-1, 80), name="نص الإشعار")
        fields_sizer.Add(self.body_input, 1, wx.EXPAND)
        
        fields_sizer.Add(wx.StaticText(self, label="الموضوع المستهدف (Topic):"), 0, wx.ALIGN_CENTER_VERTICAL)
        self.topic_input = wx.TextCtrl(self, value="all", name="موضوع الإرسال")
        fields_sizer.Add(self.topic_input, 1, wx.EXPAND)
        
        main_sizer.Add(fields_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        temp_sizer = wx.BoxSizer(wx.HORIZONTAL)
        temp_sizer.Add(wx.StaticText(self, label="القوالب الجاهزة:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.templates_combo = wx.ComboBox(self, style=wx.CB_READONLY, name="قوالب الإشعارات")
        self.templates_combo.Bind(wx.EVT_COMBOBOX, self.on_template_select)
        temp_sizer.Add(self.templates_combo, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.save_temp_btn = wx.Button(self, label="حفظ كقالب جديد", name="حفظ الرسالة كقالب")
        self.save_temp_btn.Bind(wx.EVT_BUTTON, self.on_save_template)
        temp_sizer.Add(self.save_temp_btn, 0)
        
        main_sizer.Add(temp_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        btn_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.send_btn = wx.Button(self, label="إرسال الإشعار الآن 📢", name="إرسال الإشعار لجميع الأجهزة")
        self.send_btn.Bind(wx.EVT_BUTTON, self.on_send_notification)
        btn_sizer.Add(self.send_btn, 1, wx.EXPAND)
        main_sizer.Add(btn_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        main_sizer.Add(wx.StaticText(self, label="سجل الإشعارات المرسلة:"), 0, wx.LEFT | wx.TOP, 10)
        self.history_list = wx.ListBox(self, size=(-1, 100), name="سجل الإشعارات")
        main_sizer.Add(self.history_list, 1, wx.EXPAND | wx.ALL, 10)
        
        self.SetSizer(main_sizer)
        
        self.templates_path = os.path.join(RELEASES_REPO, "fcm_templates.json")
        self.history_path = os.path.join(RELEASES_REPO, "fcm_history.json")
        ...
        if not HAS_FIREBASE:
            ...
            self.send_btn.SetLabel("يرجى تثبيت مكتبة firebase-admin أولاً")
    ...
    def load_templates(self):
        try:
            if os.path.exists(self.templates_path):
                ...
                self.templates = {
                    "إعلان تحديث": {
                        ...
                        "body": "تم إضافة ميزات وأدوات ميسرة جديدة في تطبيق محرر الفيديو الميسّر. يرجى التحديث الآن."
                    },
                    "تنبيه صيانة": {
                        "title": "صيانة مجدولة ⚙️",
                        "body": "سيتم إجراء صيانة سريعة للخدمات السحابية لمدة 10 دقائق. لن تتأثر عمليات المعالجة المحلية."
                    }
                }
            ...
            for name in self.templates.keys():
                self.templates_combo.Append(name)
        except Exception:
            self.templates = {}
        ...
        try:
            ...
            if os.path.exists(self.history_path):
                ...
                for item in self.history:
                    self.history_list.Append(f"[{item.get('time')}] {item.get('title')} ({item.get('topic')})")
            else:
                self.history = []
        except Exception:
            self.history = []
    ...
    def on_template_select(self, event):
        name = self.templates_combo.GetStringSelection()
        if name in self.templates:
            self.title_input.SetValue(self.templates[name]["title"])
            self.body_input.SetValue(self.templates[name]["body"])

    def on_save_template(self, event):
        title = self.title_input.GetValue().strip()
        body = self.body_input.GetValue().strip()
        if not title or not body:
            wx.MessageBox("يرجى ملء العنوان والرسالة لحفظهما كقالب!", "تنبيه", wx.OK | wx.ICON_WARNING)
            return
            
        dlg = wx.TextEntryDialog(self, "أدخل اسماً لهذا القالب:", "حفظ قالب")
        if dlg.ShowModal() == wx.ID_OK:
            name = dlg.GetValue().strip()
            if name:
                self.templates[name] = {"title": title, "body": body}
                try:
                    with open(self.templates_path, "w", encoding="utf-8") as f:
                        json.dump(self.templates, f, indent=2, ensure_ascii=False)
                    self.load_templates()
                    self.templates_combo.SetStringSelection(name)
                    play_success_sound()
                    wx.MessageBox("تم حفظ القالب بنجاح!", "تم الحفظ", wx.OK | wx.ICON_INFORMATION)
                except Exception as e:
                    wx.MessageBox(f"فشل حفظ القالب: {e}", "خطأ", wx.OK | wx.ICON_ERROR)
        dlg.Destroy()

    def on_send_notification(self, event):
        if not HAS_FIREBASE:
            wx.MessageBox("مكتبة firebase-admin غير مثبتة على هذا النظام!", "خطأ", wx.OK | wx.ICON_ERROR)
            return
            
        key_path = self.key_picker.GetPath()
        title = self.title_input.GetValue().strip()
        body = self.body_input.GetValue().strip()
        topic = self.topic_input.GetValue().strip() or "all"
        
        if not os.path.exists(key_path):
            play_error_sound()
            wx.MessageBox(f"لم يتم العثور على ملف المفتاح في المسار:\n{key_path}", "خطأ", wx.OK | wx.ICON_ERROR)
            return
            
        if not title or not body:
            play_error_sound()
            wx.MessageBox("يرجى ملء عنوان ونص الرسالة أولاً!", "تنبيه", wx.OK | wx.ICON_WARNING)
            return
        ...
        try:
            ...
            android_config = messaging.AndroidConfig(
                ...
                notification=messaging.AndroidNotification(
                    channel_id="accessible_fcm_channel",
                    sound="default",
                    default_sound=True,
            ...
            self.history.insert(0, {"time": dt, "title": title, "topic": topic, "response": response})
            ...
            self.load_history()
            play_success_sound()
            wx.MessageBox(f"تم إرسال الإشعار بنجاح لجميع المستخدمين! 🎉\n\nالمعرف: {response}", "نجاح الإرسال", wx.OK | wx.ICON_INFORMATION)
            
        except Exception as e:
            if wx.IsBusy():
                wx.EndBusyCursor()
            play_error_sound()
            wx.MessageBox(f"فشل إرسال الإشعار:\n{str(e)}", "خطأ في الإرسال", wx.OK | wx.ICON_ERROR)


class StringSyncDialog(wx.Dialog):
    def __init__(self, parent, missing_keys, mismatched_keys, local_strings, patch_data, patch_path):
        super().__init__(parent, title="مزامنة التراجم السحابية والمحلية", size=(650, 500))
        self.SetLayoutDirection(wx.Layout_RightToLeft)
        self.local_strings = local_strings
        self.patch_data = patch_data
        self.patch_path = patch_path
        
        sizer = wx.BoxSizer(wx.VERTICAL)
        
        info = wx.StaticText(self, label="تم اكتشاف نصوص جديدة أو معدلة محلياً في ملفات الهاتف وغير مضافة للسحابة:")
        ...
        for k, v in missing_keys:
            self.sync_items.append(('missing', k, v))
            display_items.append(f"[جديد] {k}: \"{v}\"")
            
        for k, v, old_v in mismatched_keys:
            self.sync_items.append(('mismatch', k, v))
            display_items.append(f"[معدل] {k}: \"{v}\" (السحابي: \"{old_v}\")")
            
        self.checklist = wx.CheckListBox(self, choices=display_items, name="قائمة النصوص لمزامنتها")
        for i in range(len(display_items)):
            self.checklist.Check(i, True)
            
        sizer.Add(self.checklist, 1, wx.EXPAND | wx.LEFT | wx.RIGHT, 10)
        
        sel_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.select_all_btn = wx.Button(self, label="تحديد الكل", name="تحديد كل العناصر")
        self.select_all_btn.Bind(wx.EVT_BUTTON, self.on_select_all)
        
        self.deselect_all_btn = wx.Button(self, label="إلغاء تحديد الكل", name="إلغاء تحديد كل العناصر")
        self.deselect_all_btn.Bind(wx.EVT_BUTTON, self.on_deselect_all)
        
        sel_sizer.Add(self.select_all_btn, 1, wx.EXPAND | wx.LEFT, 5)
        sel_sizer.Add(self.deselect_all_btn, 1, wx.EXPAND | wx.RIGHT, 5)
        sizer.Add(sel_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        btn_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.import_btn = wx.Button(self, label="دمج وتحديث النصوص المحددة 💾", name="دمج العناصر المحددة")
        self.import_btn.Bind(wx.EVT_BUTTON, self.on_import)
        
        self.cancel_btn = wx.Button(self, label="تخطي", name="تخطي المزامنة")
        self.cancel_btn.Bind(wx.EVT_BUTTON, lambda e: self.EndModal(wx.ID_CANCEL))
        
        btn_sizer.Add(self.import_btn, 1, wx.EXPAND | wx.LEFT, 10)
    ...
    def on_import(self, event):
        strings_dict = self.patch_data.setdefault("strings", {})
        imported_count = 0
        for i in range(self.checklist.GetCount()):
            if self.checklist.IsChecked(i):
                item_type, k, v = self.sync_items[i]
                strings_dict[k] = v
                imported_count += 1
                
        if imported_count > 0:
            current_ver = self.patch_data.get("version", 1)
            self.patch_data["version"] = current_ver + 1
            try:
                with open(self.patch_path, "w", encoding="utf-8") as f:
                    json.dump(self.patch_data, f, ensure_ascii=False, indent=2)
                play_success_sound()
                wx.MessageBox(f"تم بنجاح دمج ({imported_count}) نصوص وتحديث إصدار السحابي إلى {current_ver + 1}!", "نجاح الدمج", wx.OK | wx.ICON_INFORMATION)
            except Exception as e:
                play_error_sound()
                wx.MessageBox(f"فشل حفظ التحديثات السحابية:\n{str(e)}", "خطأ", wx.OK | wx.ICON_ERROR)
        self.EndModal(wx.ID_OK)


class LocalTranslationsTab(wx.Panel):
    def __init__(self, parent, sidebar_ref, active_repo_provider):
        super().__init__(parent)
        self.SetLayoutDirection(wx.Layout_RightToLeft)
        self.sidebar = sidebar_ref
        ...
        header = wx.StaticText(self, label="محرر الترجمات المحلية للتطبيق (strings.xml)")
        header_font = wx.Font(12, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        header.SetFont(header_font)
        main_sizer.Add(header, 0, wx.ALL | wx.ALIGN_CENTER_HORIZONTAL, 10)
        
        search_sizer = wx.BoxSizer(wx.HORIZONTAL)
        search_sizer.Add(wx.StaticText(self, label="بحث في الموارد المحلية:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.search_input = wx.TextCtrl(self, name="بحث محلي")
        self.search_input.Bind(wx.EVT_TEXT, self.on_search)
        search_sizer.Add(self.search_input, 1, wx.EXPAND | wx.RIGHT, 10)
        
        main_sizer.Add(search_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        self.list_ctrl = wx.ListCtrl(self, style=wx.LC_REPORT | wx.LC_SINGLE_SEL, name="قائمة الترجمات المحلية")
        self.list_ctrl.InsertColumn(0, "المفتاح (Key)", width=200)
        self.list_ctrl.InsertColumn(1, "النص المحلي (Local Text)", width=400)
        main_sizer.Add(self.list_ctrl, 1, wx.EXPAND | wx.ALL, 10)
        
        edit_box = wx.StaticBox(self, label="تعديل أو إضافة نص محلي")
        edit_sizer = wx.StaticBoxSizer(edit_box, wx.VERTICAL)
        
        self.sel_key_lbl = wx.StaticText(self, label="المفتاح المحلي المحدد: لا يوجد")
        edit_sizer.Add(self.sel_key_lbl, 0, wx.BOTTOM, 5)
        
        self.val_input = wx.TextCtrl(self, style=wx.TE_MULTILINE, size=(-1, 60), name="قيمة النص المحلي")
        edit_sizer.Add(self.val_input, 1, wx.EXPAND | wx.BOTTOM, 10)
        
        form_btn_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.update_btn = wx.Button(self, label="تحديث النص المحلي", name="تحديث النص المحلي")
        self.update_btn.Bind(wx.EVT_BUTTON, self.on_update_item)
        form_btn_sizer.Add(self.update_btn, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.add_new_btn = wx.Button(self, label="إضافة مفتاح محلي جديد +", name="إضافة مفتاح محلي")
        self.add_new_btn.Bind(wx.EVT_BUTTON, self.on_add_new_key)
        form_btn_sizer.Add(self.add_new_btn, 1, wx.EXPAND | wx.LEFT, 10)
        
        edit_sizer.Add(form_btn_sizer, 0, wx.EXPAND)
        main_sizer.Add(edit_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        save_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.save_btn = wx.Button(self, label="حفظ في strings.xml محلياً", name="حفظ محلياً")
        self.save_btn.Bind(wx.EVT_BUTTON, self.on_save_xml)
        save_sizer.Add(self.save_btn, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.upload_btn = wx.Button(self, label="حفظ ورفع التغييرات إلى مستودع التطبيق ☁️", name="رفع تغييرات التطبيق")
        self.upload_btn.Bind(wx.EVT_BUTTON, self.on_upload_xml)
        save_sizer.Add(self.upload_btn, 1, wx.EXPAND | wx.LEFT, 10)
        
        main_sizer.Add(save_sizer, 0, wx.EXPAND | wx.ALL, 10)
        ...
        self.load_local_strings()
        self.list_ctrl.Bind(wx.EVT_LIST_ITEM_SELECTED, self.on_item_selected)

    def get_xml_file_path(self):
        return os.path.join(SOURCE_REPO, "app", "src", "main", "res", "values-ar", "strings.xml")

    def load_local_strings(self):
        path = self.get_xml_file_path()
        ...
        try:
            ...
            for elem in root.findall('string'):
                ...
                if name:
                    ...
                    self.local_strings[name] = val
                    self.all_items.append((name, val))
                    
            self.all_items.sort()
            self.populate_list()
        except Exception as e:
            wx.MessageBox(f"فشل قراءة ملف التراجم XML:\n{e}", "خطأ", wx.OK | wx.ICON_ERROR)
    ...
    def populate_list(self, filter_text=""):
        ...
        for k, v in self.all_items:
            if not filter_text or filter_text in k.lower() or filter_text in v.lower():
                idx = self.list_ctrl.GetItemCount()
                self.list_ctrl.InsertItem(idx, k)
                self.list_ctrl.SetItem(idx, 1, v)

    def on_search(self, event):
        self.populate_list(self.search_input.GetValue())

    def on_item_selected(self, event):
        idx = event.GetIndex()
        key = self.list_ctrl.GetItemText(idx, 0)
        val = self.list_ctrl.GetItemText(idx, 1)
        self.sel_key_lbl.SetLabel(f"المفتاح المحلي المحدد: {key}")
        self.val_input.SetValue(val)

    def on_update_item(self, event):
        sel_idx = self.list_ctrl.GetNextItem(-1, wx.LIST_NEXT_ALL, wx.LIST_STATE_SELECTED)
        if sel_idx == -1:
            wx.MessageBox("الرجاء اختيار مفتاح لتعديله أولاً!", "تنبيه", wx.OK | wx.ICON_WARNING)
            return
            
        key = self.list_ctrl.GetItemText(sel_idx, 0)
        new_val = self.val_input.GetValue()
        
        self.local_strings[key] = new_val
        for i, (k, v) in enumerate(self.all_items):
            if k == key:
                self.all_items[i] = (key, new_val)
                break
                
        self.populate_list(self.search_input.GetValue())
        play_success_sound()

    def on_add_new_key(self, event):
        dlg = wx.TextEntryDialog(self, "أدخل اسم المفتاح المحلي الجديد:", "إضافة مفتاح محلي")
        if dlg.ShowModal() == wx.ID_OK:
            key = dlg.GetValue().strip()
            if not key:
                return
            if key in self.local_strings:
                wx.MessageBox("المفتاح موجود بالفعل!", "تنبيه", wx.OK | wx.ICON_WARNING)
                return
                
            dlg_val = wx.TextEntryDialog(self, f"أدخل قيمة النص لـ '{key}':", "إضافة قيمة")
            if dlg_val.ShowModal() == wx.ID_OK:
                val = dlg_val.GetValue()
                self.local_strings[key] = val
                self.all_items.append((key, val))
                self.all_items.sort()
                self.populate_list(self.search_input.GetValue())
                play_success_sound()
            dlg_val.Destroy()
    ...
    def on_save_xml(self, event):
        ...
        try:
            ...
            for k in sorted(self.local_strings.keys()):
                val = self.local_strings[k]
                val = val.replace("\\'", "'").replace('\\"', '"')
                val = val.replace("'", "\\'").replace('"', '\\"')
            ...
            if event is not None:
                wx.MessageBox("تم حفظ ملف strings.xml بنجاح!", "نجاح الحفظ", wx.OK | wx.ICON_INFORMATION)
        except Exception as e:
            play_error_sound()
            wx.MessageBox(f"فشل حفظ ملف strings.xml:\n{str(e)}", "خطأ", wx.OK | wx.ICON_ERROR)

    def on_upload_xml(self, event):
        self.on_save_xml(None)
        path = self.get_xml_file_path()
        repo_path = SOURCE_REPO
        wx.BeginBusyCursor()
        try:
            rel_xml_path = os.path.relpath(path, repo_path)
            subprocess.run(["git", "add", rel_xml_path], cwd=repo_path, check=True, capture_output=True)
            subprocess.run(["git", "commit", "-m", "Update local translations strings.xml via GUI"], cwd=repo_path, capture_output=True)
            ...
            wx.EndBusyCursor()
            play_success_sound()
            wx.MessageBox("تم رفع ملف الترجمة المحلي strings.xml إلى مستودع التطبيق بنجاح!", "تم الرفع بنجاح", wx.OK | wx.ICON_INFORMATION)
        except Exception as e:
            if wx.IsBusy():
                wx.EndBusyCursor()
            play_error_sound()
            wx.MessageBox(f"فشل رفع التحديثات إلى مستودع التطبيق:\n{str(e)}", "خطأ في الرفع", wx.OK | wx.ICON_ERROR)


class CloudTranslationsTab(wx.Panel):
    def __init__(self, parent, sidebar_ref, active_repo_provider, local_tab_ref=None):
        super().__init__(parent)
        self.SetLayoutDirection(wx.Layout_RightToLeft)
        self.sidebar = sidebar_ref
        self.active_repo_provider = active_repo_provider
        self.local_tab = local_tab_ref
        
        main_sizer = wx.BoxSizer(wx.VERTICAL)
        
        header = wx.StaticText(self, label="محرر الترجمات السحابية اليدوي (strings_patch.json)")
        header_font = wx.Font(12, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        header.SetFont(header_font)
        main_sizer.Add(header, 0, wx.ALL | wx.ALIGN_CENTER_HORIZONTAL, 10)
        
        search_sizer = wx.BoxSizer(wx.HORIZONTAL)
        search_sizer.Add(wx.StaticText(self, label="بحث في النصوص السحابية:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.search_input = wx.TextCtrl(self, name="بحث سحابي")
        self.search_input.Bind(wx.EVT_TEXT, self.on_search)
        search_sizer.Add(self.search_input, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.check_sync_btn = wx.Button(self, label="المقارنة والمطابقة مع الملف المحلي 🔄", name="مطابقة مع المحلي")
        self.check_sync_btn.Bind(wx.EVT_BUTTON, self.on_check_sync_click)
        search_sizer.Add(self.check_sync_btn, 0)
        
        main_sizer.Add(search_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        self.list_ctrl = wx.ListCtrl(self, style=wx.LC_REPORT | wx.LC_SINGLE_SEL, name="قائمة الترجمات السحابية")
        self.list_ctrl.InsertColumn(0, "المفتاح (Key)", width=200)
        self.list_ctrl.InsertColumn(1, "النص السحابي (Cloud Text)", width=400)
        main_sizer.Add(self.list_ctrl, 1, wx.EXPAND | wx.ALL, 10)
        
        edit_box = wx.StaticBox(self, label="تعديل أو إضافة نص سحابي")
        edit_sizer = wx.StaticBoxSizer(edit_box, wx.VERTICAL)
        
        self.sel_key_lbl = wx.StaticText(self, label="المفتاح السحابي المحدد: لا يوجد")
        edit_sizer.Add(self.sel_key_lbl, 0, wx.BOTTOM, 5)
        
        self.val_input = wx.TextCtrl(self, style=wx.TE_MULTILINE, size=(-1, 60), name="قيمة النص السحابي")
        edit_sizer.Add(self.val_input, 1, wx.EXPAND | wx.BOTTOM, 10)
        
        form_btn_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.update_btn = wx.Button(self, label="تحديث النص السحابي", name="تحديث النص السحابي")
        self.update_btn.Bind(wx.EVT_BUTTON, self.on_update_item)
        form_btn_sizer.Add(self.update_btn, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.add_new_btn = wx.Button(self, label="إضافة مفتاح سحابي جديد +", name="إضافة مفتاح سحابي")
        self.add_new_btn.Bind(wx.EVT_BUTTON, self.on_add_new_key)
        form_btn_sizer.Add(self.add_new_btn, 1, wx.EXPAND | wx.LEFT, 10)
        ...
        save_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.save_btn = wx.Button(self, label="حفظ ملف الترجمات السحابية محلياً", name="حفظ السحابي محلياً")
        self.save_btn.Bind(wx.EVT_BUTTON, self.on_save_patch)
        save_sizer.Add(self.save_btn, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.upload_btn = wx.Button(self, label="حفظ ورفع التغييرات إلى السحابة ☁️", name="رفع السحابي")
        self.upload_btn.Bind(wx.EVT_BUTTON, self.on_upload_patch)
        save_sizer.Add(self.upload_btn, 1, wx.EXPAND | wx.LEFT, 10)
        ...
        self.load_patch_file()
        self.list_ctrl.Bind(wx.EVT_LIST_ITEM_SELECTED, self.on_item_selected)

    def get_patch_file_path(self):
        repo_path = self.active_repo_provider.get_active_repo_path()
        if repo_path:
            path = os.path.join(repo_path, "strings_patch.json")
            if os.path.exists(path):
                return path
        for r in getattr(self.active_repo_provider, "registered_repos", []):
            alt_path = os.path.join(r["path"], "strings_patch.json")
            if os.path.exists(alt_path):
                return alt_path
        return ""

    def load_patch_file(self):
        path = self.get_patch_file_path()
        self.list_ctrl.DeleteAllItems()
        self.all_items = []
        
        if not path or not os.path.exists(path):
            self.patch_data = {"version": 1, "strings": {}}
            return
        ...
        try:
            ...
            strings_dict = self.patch_data.get("strings", {})
            for k in sorted(strings_dict.keys()):
                self.all_items.append((k, strings_dict[k]))
                
            self.populate_list()
        except Exception as e:
            wx.MessageBox(f"فشل قراءة ملف التراجم strings_patch.json:\n{e}", "خطأ", wx.OK | wx.ICON_ERROR)
    ...
    def on_item_selected(self, event):
        ...
        key = self.list_ctrl.GetItemText(idx, 0)
        val = self.list_ctrl.GetItemText(idx, 1)
        self.sel_key_lbl.SetLabel(f"المفتاح السحابي المحدد: {key}")
        self.val_input.SetValue(val)
    ...
    def on_update_item(self, event):
        ...
        self.patch_data.setdefault("strings", {})[key] = new_val
        for i, (k, v) in enumerate(self.all_items):
            if k == key:
                self.all_items[i] = (key, new_val)
                break
    ...
    def on_add_new_key(self, event):
        dlg = wx.TextEntryDialog(self, "أدخل اسم المفتاح السحابي الجديد (اسم الكلمة):", "إضافة مفتاح سحابي")
        if dlg.ShowModal() == wx.ID_OK:
            key = dlg.GetValue().strip()
            if not key:
                return
            if key in self.patch_data.setdefault("strings", {}):
                wx.MessageBox("المفتاح مدخل بالفعل!", "تنبيه", wx.OK | wx.ICON_WARNING)
                return
                
            dlg_val = wx.TextEntryDialog(self, f"أدخل قيمة النص لـ '{key}':", "إضافة قيمة")
            if dlg_val.ShowModal() == wx.ID_OK:
                val = dlg_val.GetValue()
                self.patch_data["strings"][key] = val
                self.all_items.append((key, val))
                self.all_items.sort()
                self.populate_list(self.search_input.GetValue())
                play_success_sound()
    ...
    def on_save_patch(self, event):
        ...
        if not path:
            ...
            if repo_path:
                ...
                wx.MessageBox("الرجاء اختيار مستودع نشط أولاً لحفظ الملف فيه!", "تنبيه", wx.OK | wx.ICON_WARNING)
                return
        ...
        try:
            ...
            play_success_sound()
            if event is not None:
                wx.MessageBox(f"تم حفظ ملف الترجمات السحابية محلياً! (الإصدار الجديد: {current_ver + 1})", "نجاح الحفظ", wx.OK | wx.ICON_INFORMATION)
        except Exception as e:
            play_error_sound()
            wx.MessageBox(f"فشل حفظ الملف:\n{str(e)}", "خطأ", wx.OK | wx.ICON_ERROR)

    def on_upload_patch(self, event):
        self.on_save_patch(None)
        path = self.get_patch_file_path()
        if not path:
            return
        ...
        wx.BeginBusyCursor()
        try:
            subprocess.run(["git", "add", "strings_patch.json"], cwd=repo_path, check=True, capture_output=True)
            subprocess.run(["git", "commit", "-m", "Update dynamic OTA translations via wxPython GUI"], cwd=repo_path, capture_output=True)
            ...
            wx.EndBusyCursor()
            play_success_sound()
            wx.MessageBox("تم رفع الترجمات السحابية OTA إلى مستودع GitHub بنجاح!", "تم الرفع بنجاح", wx.OK | wx.ICON_INFORMATION)
        except Exception as e:
            if wx.IsBusy():
                wx.EndBusyCursor()
            play_error_sound()
            wx.MessageBox(f"فشل رفع التحديثات إلى GitHub:\n{str(e)}", "خطأ في الرفع", wx.OK | wx.ICON_ERROR)

    def run_auto_sync_check(self):
        local_strings = {}
        if self.local_tab and getattr(self.local_tab, "local_strings", None):
            local_strings = self.local_tab.local_strings
        else:
            local_xml_path = os.path.join(SOURCE_REPO, "app", "src", "main", "res", "values-ar", "strings.xml")
            if os.path.exists(local_xml_path):
                try:
                    ...
                    for string_elem in root.findall('string'):
                        ...
                        if name:
                            val = string_elem.text or ""
                            val = val.replace("\\'", "'").replace('\\"', '"')
                            local_strings[name] = val
                except Exception:
                    pass
                    
        patch_path = self.get_patch_file_path()
        if not patch_path or not local_strings:
            return
            
        patch_strings = self.patch_data.get("strings", {})
        missing_keys = []
        mismatched_keys = []
        
        for k, v in local_strings.items():
            if k not in patch_strings:
                missing_keys.append((k, v))
            elif patch_strings[k] != v:
                mismatched_keys.append((k, v, patch_strings[k]))
                
        if missing_keys or mismatched_keys:
            with StringSyncDialog(self, missing_keys, mismatched_keys, local_strings, self.patch_data, patch_path) as dlg:
                if dlg.ShowModal() == wx.ID_OK:
                    self.load_patch_file()
        else:
            wx.MessageBox("الملف السحابي متطابق تماماً مع الملف المحلي ولا توجد نصوص ناقصة!", "مزامنة الترجمات", wx.OK | wx.ICON_INFORMATION)

    def on_check_sync_click(self, event):
        self.run_auto_sync_check()


class ComplianceAuditorTab(wx.Panel):
    def __init__(self, parent, sidebar_ref, active_repo_provider):
        super().__init__(parent)
        self.SetLayoutDirection(wx.Layout_RightToLeft)
        self.sidebar = sidebar_ref
        ...
        header = wx.StaticText(self, label="مدقق سهولة الوصول وصلاحيات الأمان التلقائي")
        header_font = wx.Font(12, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD)
        header.SetFont(header_font)
        main_sizer.Add(header, 0, wx.ALL | wx.ALIGN_CENTER_HORIZONTAL, 10)
        
        btn_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.audit_access_btn = wx.Button(self, label="فحص سهولة الوصول (Accessibility Audit) 👁️", name="تشغيل فحص الوصول")
        self.audit_access_btn.Bind(wx.EVT_BUTTON, self.on_audit_accessibility)
        btn_sizer.Add(self.audit_access_btn, 1, wx.EXPAND | wx.RIGHT, 10)
        
        self.audit_perms_btn = wx.Button(self, label="فحص صلاحيات الأمان (Permissions Audit) 🛡️", name="تشغيل فحص الصلاحيات")
        self.audit_perms_btn.Bind(wx.EVT_BUTTON, self.on_audit_permissions)
        btn_sizer.Add(self.audit_perms_btn, 1, wx.EXPAND | wx.LEFT, 10)
        
        main_sizer.Add(btn_sizer, 0, wx.EXPAND | wx.ALL, 10)
        
        self.status_lbl = wx.StaticText(self, label="اضغط على أحد أزرار الفحص لبدء التدقيق التلقائي.")
        main_sizer.Add(self.status_lbl, 0, wx.ALL, 10)
        
        self.list_ctrl = wx.ListCtrl(self, style=wx.LC_REPORT, name="نتائج التدقيق والامتثال")
        self.list_ctrl.InsertColumn(0, "الملف (File)", width=180)
        self.list_ctrl.InsertColumn(1, "الوصف (Issue)", width=450)
        self.list_ctrl.InsertColumn(2, "النوع (Type)", width=120)
        main_sizer.Add(self.list_ctrl, 1, wx.EXPAND | wx.ALL, 10)
        
        self.SetSizer(main_sizer)

    def on_audit_accessibility(self, event):
        self.list_ctrl.DeleteAllItems()
        self.status_lbl.SetLabel("جاري تشغيل مدقق سهولة الوصول...")
        wx.BeginBusyCursor()
        
        source_repo = SOURCE_REPO
        layouts_dir = os.path.join(source_repo, "app", "src", "main", "res", "layout")
        
        if not os.path.exists(layouts_dir):
            wx.EndBusyCursor()
            self.status_lbl.SetLabel("فشل: مجلد الواجهات layout غير موجود.")
            return
        ...
        try:
            for file_name in os.listdir(layouts_dir):
                if not file_name.endswith(".xml"):
                    continue
                file_path = os.path.join(layouts_dir, file_name)
                try:
                    ...
                    def scan_elem(elem):
                        ...
                        if tag in ["ImageButton", "ImageView"]:
                            important = elem.attrib.get('{http://schemas.android.com/apk/res/android}importantForAccessibility')
                            if important != "no":
                                content_desc = elem.attrib.get('{http://schemas.android.com/apk/res/android}contentDescription')
                                if not content_desc or content_desc.startswith("@null"):
                                    issues.append((file_name, f"عنصر <{tag}> يفتقد لوصف قارئ الشاشة (contentDescription)", tag))
                        elif tag in ["EditText", "TextInputEditText"]:
                            hint = elem.attrib.get('{http://schemas.android.com/apk/res/android}hint')
                            if not hint:
                                issues.append((file_name, f"حقل إدخال <{tag}> يفتقد للتلميح أو التسمية (hint)", tag))
                        for child in elem:
                            scan_elem(child)
                    scan_elem(root)
                except Exception as e:
                    issues.append((file_name, f"خطأ أثناء التحليل: {e}", "خطأ"))
        except Exception as e:
            issues.append(("عام", f"فشل فحص الواجهات: {e}", "خطأ"))
            
        wx.EndBusyCursor()
        if issues:
            self.status_lbl.SetLabel(f"اكتمل الفحص: تم العثور على ({len(issues)}) مشاكل يجب إصلاحها لسهولة الوصول.")
            for file, desc, item_type in issues:
                idx = self.list_ctrl.GetItemCount()
                self.list_ctrl.InsertItem(idx, file)
                self.list_ctrl.SetItem(idx, 1, desc)
                self.list_ctrl.SetItem(idx, 2, item_type)
            play_error_sound()
        else:
            self.status_lbl.SetLabel("تهانينا! لم يتم العثور على أي مشاكل. واجهات التطبيق متوافقة بنسبة 100% مع قارئ الشاشة!")
            play_success_sound()

    def on_audit_permissions(self, event):
        self.list_ctrl.DeleteAllItems()
        self.status_lbl.SetLabel("جاري تشغيل مدقق صلاحيات الأمان...")
        wx.BeginBusyCursor()
        
        source_repo = SOURCE_REPO
        java_dir = os.path.join(source_repo, "app", "src", "main", "java", "com", "example", "accessiblevideoeditor")
        
        if not os.path.exists(java_dir):
            wx.EndBusyCursor()
            self.status_lbl.SetLabel("فشل: مجلد كود Java/Kotlin غير موجود.")
            return
            
        issues = []
        permission_regex = re.compile(r'permission\.(CAMERA|RECORD_AUDIO|WRITE_EXTERNAL_STORAGE|READ_EXTERNAL_STORAGE|READ_MEDIA_VIDEO|READ_MEDIA_AUDIO|POST_NOTIFICATIONS)', re.IGNORECASE)
        check_regex = re.compile(r'checkSelfPermission', re.IGNORECASE)
                ...
                for file in files:
                    ...
                    try:
                        with open(file_path, "r", encoding="utf-8") as f:
                            content = f.read()
                        matches = permission_regex.findall(content)
                        if matches:
                            if not check_regex.search(content):
                                issues.append((file, f"يستخدم صلاحيات {list(set(matches))} ولكنه لا يحتوي على فحص صلاحيات ديناميكي (checkSelfPermission)", "أمن خطر"))
                    except Exception as e:
                        issues.append((file, f"خطأ أثناء قراءة الملف: {e}", "خطأ"))
        except Exception as e:
            issues.append(("عام", f"فشل فحص الصلاحيات: {e}", "خطأ"))
            
        wx.EndBusyCursor()
        if issues:
            self.status_lbl.SetLabel(f"اكتمل الفحص: تم العثور على ({len(issues)}) كود خطر يفتقد للتحقق من صلاحية التشغيل.")
            for file, desc, item_type in issues:
                idx = self.list_ctrl.GetItemCount()
                self.list_ctrl.InsertItem(idx, file)
                self.list_ctrl.SetItem(idx, 1, desc)
            ...
            self.status_lbl.SetLabel("تهانينا! لم يتم العثور على أي ملف خطر. فحص صلاحيات الأمان متوافق بنسبة 100%!")
            play_success_sound()
...
class MainManagerFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        self.tab9 = FCMNotificationTab(self.notebook, self.sidebar, active_repo_provider=self)
        self.tab10 = LocalTranslationsTab(self.notebook, self.sidebar, active_repo_provider=self)
        self.tab11 = CloudTranslationsTab(self.notebook, self.sidebar, active_repo_provider=self, local_tab_ref=self.tab10)
        self.tab12 = ComplianceAuditorTab(self.notebook, self.sidebar, active_repo_provider=self)
        ...
        self.notebook.AddPage(self.tab9, "9. مرسل الإشعارات")
        self.notebook.AddPage(self.tab10, "10. الترجمات المحلية (strings.xml)")
        self.notebook.AddPage(self.tab11, "11. الترجمات السحابية (strings_patch.json)")
        self.notebook.AddPage(self.tab12, "12. مدقق التوافقية والأمان")
    ...
    def active_repo_changed(self):
        ...
        self.tab10.load_local_strings()
        self.tab11.load_patch_file()
    ...
    def on_tab_changed(self, event):
        ...
        if sel in (0, 1, 4, 5, 6, 7, 8, 9, 10, 11):
