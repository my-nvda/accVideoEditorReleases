...
Object.keys(translations).forEach(lang => {
    const filePath = path.join(baseDir, `values-${lang}`, 'strings.xml');
    ...
    for (const key in trans) {
        let value = trans[key].replace(/'/g, "\\'").replace(/"/g, '\\"');
        const regex = new RegExp(`<string name="${key}">([\\s\\S]*?)</string>`);
        if (regex.test(content)) {
            content = content.replace(regex, `<string name="${key}">${value}</string>`);
        } else {
            // Append right before </resources>
